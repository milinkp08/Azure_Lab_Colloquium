...
(for example [sample1\_Multiplication.cfg](./sample1_Multiplication.cfg))
...
#### [sample1\_Multiplication.py](./sample1_Multiplication.py)
The Multiplication sample is an introduction to the framework required to
communicate with the Batch service. It submits a job using an auto-pool and
then submits a task which performs a simple arithmetic operation. The task has no
...
This sample expands on the Multiplication sample. It creates a fixed pool and then
